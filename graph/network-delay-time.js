...
var networkDelayTime = function (times, n, k) {
  ...
  dists[k - 1] = 0; // Nodes are 1-indexed. So subtract 1 for the index into the array.
  let updated = true;
  while (updated) {
    updated = false;
    for (const [s, t, w] of times) {
      if (dists[s - 1] + w < dists[t - 1]) {
        dists[t - 1] = dists[s - 1] + w;
        updated = true;
      }
    }
  }
...
//         const li = 2*pi + 1, ri = 2*pi + 2;
//         if (li >= len) return;
//         const pval = this.vals[pi], lval = this.vals[li], rval = this.vals[ri];
