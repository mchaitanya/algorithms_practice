...
var lowestCommonAncestor = function (root, p, q) {
  let lca = null;
  // Return 2 values - whether the subtree contains p & q.
  function dfs(node) {
    if (node == null) return [false, false];
    const [leftContainsP, leftContainsQ] = dfs(node.left);
    const [rightContainsP, rightContainsQ] = dfs(node.right);
    const containsP = leftContainsP || rightContainsP || node.val === p.val;
    const containsQ = leftContainsQ || rightContainsQ || node.val === q.val;
    if (lca == null && containsP && containsQ) {
      lca = node;
    }
