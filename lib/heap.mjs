...
Heap.prototype.extract = function() {
    ...
    this._vals.unshift(this._vals.pop()); // replace the root with the right-most node on the last level
...
Heap.prototype._bubble = function(idx, dir) {
    ...
    const ldx = 2*pdx + 1;
    const rdx = 2*pdx + 2;
    if ((ldx < this._vals.length && !this._comparatorFn(this._vals[pdx], this._vals[ldx])) || (rdx < this._vals.length && !this._comparatorFn(this._vals[pdx], this._vals[ldx]))) {
